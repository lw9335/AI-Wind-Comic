...
ComfyUI 是可选的本地图片工作流引擎，不是默认出图路径。Wind Comic 通过它做角色 IP-Adapter 一致性和可选的 ControlNet 草图硬锁。ControlNet 不是又一个云厂商：它用 Canny 边缘图按分镜草图硬锁构图、机位和主体位置，比把草图当普通参考图更硬；未配置 `COMFYUI_CONTROLNET_MODEL` 时不会走这条路径。概念与代码口径见 [00 总览 2.1](./00_WindComic项目解析总览.md) 和 [02 图片 Provider](./02_AI服务与Provider路由机制.md)。
